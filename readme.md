...
이미지의 고수준 특징을 추출하기 위해 다운샘플링이 사용된다. 다운샘플링을 진행하면, 이미지의 고수준 추상화된 정보를 얻게 된다. 이미지의 공간적인 해상도가 감소하지만, 보다 추상화된 특징을 얻을 수 있다.

## 업샘플링(upsampling)이란?

다운샘플링된 특징을 기반으로 입력 이미지의 원래 크기로 복원하기 위해 업샘플링이 사용된다. 업샘플링을 사용해 공간적인 해상도를 키워 인코더(다운샘플링)에서 추출한 특징과 결합되어 세분화된 분할 결과를 얻을 수 있다.
